...
class ExtraordinaryReportProcessor(BaseDocumentProcessor):
    ...
    def process(self) -> Optional[StructuredDocumentData]:
        ...
        logger.debug(
            f"Finished processing Extraordinary Report {self.doc_id}. Extracted {len(key_facts)} key facts and {len(structured_data['text_blocks'])} text blocks."
        )
...
class GenericReportProcessor(BaseDocumentProcessor):
    ...
    def process(self) -> Optional[StructuredDocumentData]:
        ...
        logger.debug(
            f"Finished processing Generic Report {self.doc_id}. Extracted {len(structured_data['text_blocks'])} text blocks."
        )
        return structured_data if structured_data else None
